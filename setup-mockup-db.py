...
cursor.execute(('CREATE TABLE team_members( '
                   'tid INTEGER REFERENCES teams(id), '
                   'sid VARCHAR(20) REFERENCES students(id), '
                   'session_id INTEGER REFERENCES capstone_session(id), '
...
cursor.execute(('CREATE TABLE reports('
                   'time TIME, '
                   'session_id INTEGER REFERENCES capstone_session(id), '
                   'reporting VARCHAR(20) REFERENCES students(id), '
                   'tid INTEGER REFERENCES teams(id), '
                   'report_for VARCHAR(20) REFERENCES students(id), '
    ...
    for id_number in teams["Team " + str(student[3])]:
        ...
        cursor.execute(('INSERT INTO reports '
                        'VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'),
                           (0, # time
                            2, # session id
                            new_midterm["reporting"], 
                            new_midterm["tid"], 
                            new_midterm["report_for"], 
                            new_midterm["tech_mastery"], 
                            new_midterm["work_ethic"], 
                            new_midterm["communication"], 
                            new_midterm["cooperation"], 
                            new_midterm["initiative"], 
                            new_midterm["team_focus"], 
                            new_midterm["contribution"], 
                            new_midterm["leadership"], 
                            new_midterm["organization"], 
                            new_midterm["delegation"], 
                            new_midterm["points"], 
                            new_midterm["strengths"], 
                            new_midterm["weaknesses"], 
                            new_midterm["traits_to_work_on"], 
                            new_midterm["what_you_learned"], 
                            new_midterm["proud_of_accomplishment"], 
                            new_midterm["is_final"]))
        cursor.execute(('UPDATE students SET midterm_done = ? WHERE id = ? '
                         'AND session_id = ?'), (True, student[2], 2))
        ...
        cursor.execute(('INSERT INTO reports '
                        'VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'),
                           (0, # time
                            ...
                            new_final["reporting"],
                            new_final["tid"],
                            new_final["report_for"],
                            new_final["tech_mastery"],
                            new_final["work_ethic"],
                            new_final["communication"],
                            new_final["cooperation"],
                            new_final["initiative"],
                            new_final["team_focus"],
                            new_final["contribution"],
                            new_final["leadership"],
                            new_final["organization"],
                            new_final["delegation"],
                            new_final["points"],
                            new_final["strengths"],
                            new_final["weaknesses"],
                            new_final["traits_to_work_on"],
                            new_final["what_you_learned"],
